#!/usr/bin/env python3
"""
Training script for Transformer1 MNIST model.
"""
...
def train_model(model, train_loader, test_loader, device, epochs=10, learning_rate=0.001, 
    ...
    """
    Train the Transformer1 MNIST model.
    ...
    for epoch in range(epochs):
        ...
        if accuracy > best_accuracy:
            ...
            checkpoint_path = os.path.join(checkpoint_dir, "transformer1_mnist.pt")
            torch.save({
                'epoch': epoch,
        ...
        checkpoint_path = os.path.join(checkpoint_dir, f"transformer1_mnist_epoch_{epoch+1}.pt")
        torch.save({
            'epoch': epoch,
            'model_state_dict': model.state_dict(),
            'optimizer_state_dict': optimizer.state_dict(),
...
def main():
    """Main function to train the Transformer1 MNIST model."""
    parser = argparse.ArgumentParser(description="Train Transformer1 MNIST Model")
    ...
    export_path = os.path.join(args.checkpoint_dir, "transformer1_mnist.pt")
